...
class BookingConfirmResolveWizard(models.TransientModel):
    ...
    def _notify_stakeholders(self, booking):
        ...
        if booking.shipment_id:
            template = self.env.ref('viin_freight.email_template_shipment_carrier_resolution', raise_if_not_found=False)
        else:
            template = self.env.ref('viin_freight.email_template_carrier_resolution', raise_if_not_found=False)
