...
class Shipment(models.Model):
    ...
    def _get_cost_billed_amounts(self, line):
        ...
        This can be overridden in other modules (e.g., viin_freight_project)
    ...
    def action_view_bookings(self):
        ...
        if len(self.booking_ids) == 1:
            form = self.env.ref('viin_freight.freight_booking_view_form')
    ...
    def action_view_progress(self):
        ...
        self.ensure_one()
        action = self.env['ir.actions.act_window']._for_xml_id('viin_freight.action_freight_shipment_progress_report')
    ...
    def action_view_profitability(self):
        ...
        self.ensure_one()
        action = self.env['ir.actions.act_window']._for_xml_id('viin_freight.action_shipment_profitability_report')
    ...
    def action_update_shipment_tracking(self):
        ...
        return {
            ...
            'view_mode': 'form',
            'view_id': self.env.ref('viin_freight.shipment_tracking_form_view').id,
            'target': 'new',
            'context': {
    ...
    def action_view_shipment_tracking(self):
        self.ensure_one()
        action = self.env['ir.actions.act_window']._for_xml_id('viin_freight.action_shipment_tracking')
        action['context'] = {
            'default_shipment_id': self.id,
            'default_stage_id': self.current_stage_id.id,
            'default_notify_to_partner_ids': self.third_party_ids.ids,
