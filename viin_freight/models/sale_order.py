...
class SaleOrder(models.Model):
    ...
    booking_ids = fields.One2many('freight.booking', 'sale_order_id', string='Bookings', groups="viin_freight.group_freight_user")
    booking_count = fields.Integer(string='Bookings Count', compute='_compute_booking_count', groups="viin_freight.group_freight_user")
    shipment_ids = fields.One2many('freight.shipment', 'sale_order_id', string='Shipments', groups="viin_freight.group_freight_user")
    shipment_id = fields.Many2one('freight.shipment', string='Shipment', groups="viin_freight.group_freight_user",
        domain="[('sale_order_id', '=', False)]", help="Choose the existing shipment for this sales order to be used for freight services.")
    shipment_count = fields.Integer(string='Shipments Count', compute='_compute_shipment_count', groups="viin_freight.group_freight_user")

    freight_route_ids = fields.One2many('freight.route', 'sale_order_id', string='Routes', groups="viin_freight.group_freight_user")
    ...
    def action_view_bookings(self):
        action = self.env['ir.actions.act_window']._for_xml_id('viin_freight.action_freight_booking')
    ...
    def action_view_shipments(self):
        action = self.env['ir.actions.act_window']._for_xml_id('viin_freight.action_freight_shipment')
        context = {}
        context.update({
        ...
        if self.shipment_id:
            form = self.env.ref('viin_freight.freight_shipment_form_view')
    ...
    def _action_cancel(self):
        ...
        if self.env.user.has_group('viin_freight.group_freight_user'):
    ...
    def _action_confirm(self):
        ...
        safe_write_context = self.sudo() if not self.env.user.has_group('viin_freight.group_freight_user') else self
    ...
    def _activity_cancel_on_shipment(self):
        ...
        for shipment in shipments:
            shipment._activity_schedule_with_view('mail.mail_activity_data_warning',
                ...
                views_or_xmlid='viin_freight.exception_sale_order_on_shipment_cancellation',
                render_context={
